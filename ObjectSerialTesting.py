...
def main(arg):
    ...
    #     print "ACL:\n"+repr(key2.get_acl())
        content_string=key2.get_contents_as_string(cb=percent_cb,num_cb=10,headers={"testdown":"load"},response_headers={"Content-Type":"text/plain"})
    #     print "\nGet file content as string..\n    "+content_string
        
        fp = open('D:/download.test', 'w')
        key.get_contents_to_file(fp,cb=percent_cb,num_cb=10,headers={"testdown":"load"},response_headers={"Content-Type":"text/plain"})
        key.get_contents_to_filename('D:/download.test2',cb=percent_cb,num_cb=10,headers={"testdown":"load"},response_headers={"Content-Type":"text/plain"})
        key.get_file(fp,cb=percent_cb,num_cb=10,headers={"testdown":"load"},override_num_retries=2,response_headers={"Content-Type":"text/plain"})
       
        f2 = open('D:/testwrite', 'w+')
        f2.write("Hello")
        key.set_contents_from_file(f2,replace=True,cb=percent_cb,num_cb=2,rewind=True,policy='public-read')
        f2.close()
        key.set_contents_from_filename('D:/download.test2',replace=True,cb=percent_cb,num_cb=2,policy='public-read')
